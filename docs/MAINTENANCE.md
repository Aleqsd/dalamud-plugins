...
Hotbar Atelier et Codex Monitor livrent actuellement leur DLL sous `plugin/` ; le script la place à la racine du ZIP d'installation. Codex Monitor embarque son relais dans sa DLL : la 0.11.0 contient cinq scripts (`bridge.mjs`, `observer.mjs`, `questions.mjs`, `usage.mjs`, `files.mjs`), sans fichier de relais supplémentaire dans le ZIP installateur. Vérifier que chaque ressource correspond aux sources de la release, y compris les nouveaux modules.
