...
Hotbar Atelier et Codex Monitor livrent actuellement leur DLL sous `plugin/` ; le script la place à la racine du ZIP d'installation. Aether Compass et Cycle & Opener livrent leurs DLL, manifestes et dépendances à la racine de leurs archives sources ; Aether Compass conserve aussi sa DLL Core. Codex Monitor embarque son relais dans sa DLL : la 0.11.2 contient cinq scripts (`bridge.mjs`, `observer.mjs`, `questions.mjs`, `usage.mjs`, `files.mjs`), identiques à ceux de la 0.11.0, sans fichier de relais supplémentaire dans le ZIP installateur. Vérifier que chaque ressource correspond aux sources de la release, y compris les nouveaux modules.
...
Aether Compass utilise le dépôt source `Aleqsd/aether-compass`, l'identité `AetherCompass` et les commandes `/goals` et `/aethercompass`. Son archive source contient `AetherCompass.dll`, `AetherCompass.Core.dll`, `AetherCompass.deps.json`, `AetherCompass.json` et sa licence. Le catalogue conserve les deux DLL et ajoute son icône. Le suivi des récompenses reste local, séparé par personnage et inconnu lorsqu'aucune preuve d'acquisition n'est disponible ; ne pas le présenter comme une lecture historique des droits de butin.
